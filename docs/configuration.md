...
| `uri`                    | `AUTO_CONFIG_URI`                     |   URI   | _(8)_   | URI for the service that provides the automatic configuration stream. If set, this overrides `streamUri` for the automatic configuration stream only. _(8)_                                                                          |
...
_(8)_ The automatic configuration stream normally comes from the same service as the SDK streams, so the default is whatever `streamUri` in the `[Main]` section is set to. Set `uri` only if the automatic configuration stream is served from a different location; the SDK streams for each environment continue to use `streamUri`. As with `streamUri`, you should never need to set this unless you are using a special instance of the LaunchDarkly service or accessing LaunchDarkly through a reverse proxy.
